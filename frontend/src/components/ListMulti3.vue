...
<nav class="overflow-auto">
  <div class="text-sm font-semibold inline-block">
    <div class="text-xl font-normal text-center">
      Использование товаров, изготовленных (полученных) на территории свободного склада с использованием  товаров, помещенных под
      таможенную процедуру свободного склада
    </div>
  </div>
...
<table class="w-full">
  ...
  <thead>
    ...
    <tr class="h-8 bg-blue-400 text-sm font-semibold text-white text-center">
      ...
      <td class="border">
        <div class="px-2 py-2 ">
          номер и дата заключения (при наличии)
        </div>
      </td>
      <td class="border">
        <div class="px-2 py-2 ">
    ...
    <!-- col numbers level -->
    <tr class="h-8 bg-blue-400 text-sm font-semibold text-white text-center">
      <td class="border" v-for="n in Array.from({length: 14}, (_, i) => i + 1)"><div class="px-2 py-2" >{{ n }}</div></td>
    </tr>
  ...
  <tbody>
    ...
    <tr class="border-t text-xs font-normal text-center cursor-pointer hover:bg-gray-100" >
      <td class="" v-for="n in [...Array(14).keys()]"><div class="px-2 py-2 min-w-max">-</div></td>
    </tr>
